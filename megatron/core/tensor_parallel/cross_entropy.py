...
class _VocabParallelCrossEntropy(torch.autograd.Function):
    ...
    def forward(ctx, vocab_parallel_logits, target, label_smoothing=0.0):
        ...
        arange_1d = torch.arange(start=0, end=logits_2d.size()[0],
                                 ...
                                 dtype=torch.int)
        predicted_logits_1d = logits_2d[arange_1d.long(), masked_target_1d]
    ...
    def backward(ctx, grad_output):
        ...
        arange_1d = torch.arange(start=0, end=grad_2d.size()[0],
                                 device=grad_2d.device, dtype=torch.int)

        softmax_update = target_mask.view(-1).float()

        if label_smoothing > 0:
            # XLA friendly + long conversion
            smoothing = label_smoothing * vocab_size / (vocab_size - 1)
            grad_2d[arange_1d.long(), masked_target_1d.long()] -= \
