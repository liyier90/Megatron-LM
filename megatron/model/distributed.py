...
def bucket_allreduce(tensor_list):
    bucket_cap = int(os.getenv('BUCKET_CAP_KB', _ALLREDUCE_BUCKET_CAP_MB)) \
                 * 1024 * 1024
    ...
    for grad in gradients:
        ...
        if grad_bytes > bucket_cap:
            ...
            if len(tensor_bucket):
                ...
                total = 0
                tensor_bucket = []
            xm.all_reduce('sum', [grad],
                          groups=mpu.get_data_parallel_group()._mesh)
            continue

        # Bucketize till the total spills over
        total += grad_bytes
        if total > bucket_cap:
            xm.all_reduce('sum', tensor_bucket,
                          groups=mpu.get_data_parallel_group()._mesh)
            total = grad_bytes
            tensor_bucket = []
